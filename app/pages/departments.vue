  ...
  <div class="bg-ivory overflow-hidden">
    ...
    <section class="relative h-[55vh] min-h-[400px] flex items-center justify-center text-white">
      ...
      <div class="relative z-10 container mx-auto px-6 text-center max-w-4xl animate-fade-in-up">
        ...
        <h1 class="text-5xl md:text-6xl font-display font-medium leading-tight mb-4">Find Your Unit</h1>
    ...
    <section class="container mx-auto px-6 py-20">
      ...
      <div v-else>
        <div v-if="departments.length === 0" class="text-center py-20 bg-white rounded-3xl border border-warmGray-100">
          <p class="text-warmGray-400">No departments available yet.</p>
        </div>
        
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div v-for="dept in departments" :key="dept._id" class="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-black/5 transition-all duration-500 group border border-warmGray-100 flex flex-col h-full hover:-translate-y-1">
            <div class="h-56 relative overflow-hidden shrink-0">
              ...
              <div class="absolute bottom-6 left-6 right-6">
                <h3 class="text-2xl font-display font-bold text-white leading-tight">{{ dept.name }}</h3>
              </div>
            </div>
            ...
            <div class="p-8 flex-grow flex flex-col justify-between bg-white relative">
              ...
              <div class="pt-5 border-t border-warmGray-100 mt-auto flex items-center justify-between">
                <div>
                  ...
                  <p class="text-warmGray-900 font-semibold text-sm">{{ dept.leaderName || 'To Be Announced' }}</p>
                </div>
